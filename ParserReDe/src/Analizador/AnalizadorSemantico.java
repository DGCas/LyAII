		...
		switch(token){
			...
			case "print":
				...
				if(simbolos.get(val1) == null){
					...
					catch(Exception err){throw new SemanticError("--Error Semantico--  "+ val1 +" no existe, operaci�n no puede completarse");}
				}
				...
				if(simbolos.get(val2) == null){
					...
					//Si no puede parsear a entero tira una exepcion.
					catch(Exception err){throw new SemanticError("--Error semantico-- "+ val2 + " no existe, imposible completar operaci�n");}
				}
